...
// Bolder and Larger LCD Subpixels (from v9)
...
float getWaterHeight(vec2 uv) {
    ...
    // (변경점) 캔버스 알파 감소가 더욱 빠른 수축(Shrink) 셰이프 변화를 
    // 일으키도록 Smoothstep 임계 구간을 살짝 좁힘
    return smoothstep(0.04, 0.25, rawH) * rawH;
}
...
const RGBWaterPlane = () => {
    ...
    useFrame(() => {
        ...
        if (ctx && texRef.current) {
            ...
            ctx.filter = 'none';
            // 수축되는 증발 형태를 뚜렷하게 만들기 위한 알파 감쇠 조정.
            // 가장자리가 깎이면서 안쪽으로 말려들어가는(Shrinking) 효과가 발생함
    ...
    const splashWater = (uv: THREE.Vector2) => {
        ...
        for (let i = 0; i < numDrops; i++) {
            ...
            // 그라데이션 중심에서 외곽으로의 감쇠율을 선형 구조로 주어,
            // 캔버스가 어두워질 때(증발할 때) 외곽부터 순차적으로 깔끔하게 쪼그라드는 효과 부여
...
const RGBWater13: React.FC = () => {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const triggerExport = useExport(canvasRef, 'rgb-water-drops-v13.png') as () => void;
