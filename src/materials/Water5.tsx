...
void main() {
    ...
    // Calculate wave intensity based on how much the normal deflects from pure flat
    float waveIntensity = smoothstep(0.01, 0.4, length(normalOffset));
    
    // Light calculation for subtle grey shading
    vec3 lightDir = normalize(vec3(0.5, 0.7, 1.0));
    float ndotl = dot(N, lightDir);
    // Darken areas that face away from the imaginary light source
    float shadow = smoothstep(1.0, 0.5, ndotl);
    
    // White background -> grey waves
    vec3 waveColor = mix(vec3(1.0), vec3(0.7, 0.7, 0.73), shadow);
    
    // Mix the flat white canvas with the grey wave sections
    ...
    // Add bright glints strictly on the waves
...
const WaterMaterial5: React.FC = () => {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const triggerExport = useExport(canvasRef, 'scattered-puddle-v5.png') as () => void;
