...
vec3 getLCDColor(vec2 uv) {
    vec2 p = uv * u_resolution.xy * 0.08; 
...
float getWaterHeight(vec2 uv) {
    // scale coordinates, NO time animation so drops are permanently static
...
void main() {
    ...
    // Also requires physical slope (so the un-water screen remains white)
    float rgbMinSlope = smoothstep(0.02, 0.12, slope);
    
    // Add chunky patchwork noise for irregular color blooming
    ...
    // Clear water center for large flat drops
...
const RGBWaterPlane = () => {
    ...
    useFrame(() => {
        ...
        if (ctx && texRef.current) {
            // Slow evaporation so large splatters stay a while
    ...
    const splashWater = (uv: THREE.Vector2) => {
        ...
        // Number of splattered drops per click
        const numDrops = 15 + Math.floor(Math.random() * 20);
        
        for (let i = 0; i < numDrops; i++) {
            // Drops scatter outwards, clustered near the center
            const angle = Math.random() * Math.PI * 2;
            const distRatio = Math.pow(Math.random(), 2.0); 
            const distance = distRatio * 180.0; // max scatter radius
            ...
            let r;
            if (i < 3) {
                // 3 Large central/random blobs
                r = 25 + Math.random() * 40; 
            } else {
                // Numerous small satellite droplets, shrinking the further they fly
                r = 2 + Math.random() * 15 * (1.0 - distRatio); 
            }
            
            const grad = ctx.createRadialGradient(x, y, 0, x, y, r);
            
            // Build the water height domes. Additive blending allows them to merge into gooey puddles.
...
const RGBWater7: React.FC = () => {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const triggerExport = useExport(canvasRef, 'rgb-water-drops-v7.png') as () => void;
